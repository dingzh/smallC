...
char const* tempname = "temp";
char const* varname = "var";
char const* labelname = "l";
...
int fetch_register(Address ptr, int k) {
    if (ptr.value == -2) {
 	return 4;
    }
    if (ptr.value == -3) {
...
void store_register(char const* s, int reg) {
    if (reg == -2) {
 	printf("move $a0, %s\n", s);
    }
    if (reg == -3) {
...
void generate_mips(Quadruple tuple,int delta) {
	//cout << "tuple.op ";
	//cout << tuple.op << endl;
	string s = tuple.op;
	int i;
	...
	if (s=="label") {
        	...
        	int src = fetch_register(tuple.arguments[0], 8); //$t0
        	printf("%s $%d, %s%d\n", s.c_str(), src, labelname, tuple.arguments[1].value);
        		...
        		int src1 = fetch_register(tuple.arguments[1], 9);
        		if (tuple.arguments[2].type == ADDRESS_CONSTANT) {
            			printf("%s $%d, $%d, %d\n", s.c_str(), dest, src1, tuple.arguments[2].value);
        		} else {
            			int src2 = fetch_register(tuple.arguments[2], 10);
            			printf("%s $%d, $%d, $%d\n", s.c_str(), dest, src1, src2);
        		}
        		if (dest == 8) {
            			store_register("$8", tuple.arguments[0].value);
        		}
    	} else if (s=="neg"|| s=="not") {
        	int src = fetch_register(tuple.arguments[1], 9);
        	int dest = get_register_id(tuple.arguments[0], 8);
       		printf("%s $%d, $%d\n", s.c_str(), dest, src);
        	if (dest == 8) {
            		store_register("$8", tuple.arguments[0].value);
        	...
        	int src = fetch_register(tuple.arguments[0], 8);
       	 	int dest = fetch_register(tuple.arguments[1], 9);
