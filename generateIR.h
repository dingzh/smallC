...
Quadruple new_quadruple_arithmetic(char const*s, int ra, int rb, int rc);
void translate_exps(TreeNode *p, int reg);
...
Quadruple new_quadruple_call(char const*s);
Quadruple new_quadruple_branch(char const*s, int a, int b);
...
Quadruple new_quadruple_arithmetic_unary(char const*s, int ra, int rb) {
	Quadruple ret = new_quadruple();
	ret.op = s;
	ret.arguments[0].type = ADDRESS_TEMP;
	ret.arguments[0].value = ra;
...
void translate(TreeNode *p) {
	...
	switch(p->type) {
		...
		case _EXTDEFS: translate_extdefs(p);break;
...
void translate_exps(TreeNode *p, int reg) {
	if (p->type == _INT) {
		IR_push_back(new_quadruple_li(reg, strtol(p->data, NULL, 0)));
			set_register_state_to_value(reg);
	}
